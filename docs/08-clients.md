...
| `oag/auto` is missing from `/v1/models` | hidden by default: the classifier behind it picks a rung from the request's shape, not its difficulty, so the name promises more than it delivers. Still resolvable — an existing pin keeps working | set `gateway.advertise_auto: true` to advertise it again; `oag/<rung>` is listed either way |
